...
server = server.replace(/app\.get\("\/api\/hydrants\/export", \(req, res\) => \{[\s\S]*?\n\}\);\n\napp\.get\("\/api\/tests"/, `app.get("/api/hydrants/export", (req, res) => {
